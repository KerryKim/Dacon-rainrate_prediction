...
def generate_split():
    ...
    files = glob.glob('data/train_normalization/*.npy')
    for fl in files:
...
if __name__ == "__main__":
    ...
    #Make test data
    test_path = 'data/test_normalization'
    ...
    for fold in range(nfolds):
        ...
        #sgd = SGD(lr=1e-4, decay=1e-6, momentum=0.9) #decay=1e-6, momentum=0.9, nesterov=True
        adam = Adam(lr=1e-4)
        adam = tfa.optimizers.SWA(adam, start_averaging=10, average_period=2)
